-- генерация движения товара за сегодня для обычных магазинов - овощи
DO $$DECLARE r record;
d_start timestamp;
d_end timestamp;
is_active int4;
...
begin
    ...
    -- отбор по дням
    <<days>>
   while d_start_day <= date_trunc('day', current_date) loop
...
select
	...
	count(*) over (
	order by g.id ) rw,
	max(g.id) over (partition by g.units_id) max_id,
	min(g.id) over (partition by g.units_id) min_id
from
	goods g,
	price_goods pg
where
	...
	and (current_date between pg.date_start and pg.date_end)
	and g.type_goods_id = 2 ) a1
 
	loop
    
    n_quantity_diff := r.cnt_rw - 2;
    -- случайная генерация
    n_quantity := ceil(random() * n_quantity_max);
    case
      when r.good_name = 'Кабачки' then 
        n_quantity := ceil(n_quantity * 0.5);
    ...
    INSERT INTO shops_wr_goods (date_create, shop_id, good_id, price_id, quantity, unit_id, is_active) VALUES(d_start_day, n_shop_id, r.good_id, r.price_id, n_quantity, r.unit_id, 0);
   END loop goods;
